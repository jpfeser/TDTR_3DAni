...
lambda_tensor(3,2) = 12; %change to 3D anisotropic for some layers
C = [2.65 0.1 2.0]*1e6;
h = [36.6 1 1e6]*1e-9;
...
lambda_perp_vect = linspace(5,20,15) %conditions to simulate

for i = 1:length(lambda_perp_vect)
    lambda_tensor(3,2) = lambda_perp_vect(i); % conditions to simulate, a-dir (low k), 

%% First calculate concentric case
xoffset = 0;
yoffset = 0;

Vout_max = TDTR_3DAni_getVout_wOffset_v2(tdelay,lambda_tensor,C,h,f,tau_rep,wp_x,wp_y,Qp,ws_x,ws_y,TCR,xoffset,yoffset);
...
x_HWHM(i) = fzero(@(x) 0.5*Vout_max - TDTR_3DAni_getVout_wOffset_v2(tdelay,lambda_tensor,C,h,f,tau_rep,wp_x,wp_y,Qp,ws_x,ws_y,TCR,x,yoffset),[xmin_guess,xmax_guess],optimset('Display','iter','TolX',1e-9))
...
y_HWHM(i) = fzero(@(x) 0.5*Vout_max - TDTR_3DAni_getVout_wOffset_v2(tdelay,lambda_tensor,C,h,f,tau_rep,wp_x,wp_y,Qp,ws_x,ws_y,TCR,xoffset,x),[ymin_guess,ymax_guess],optimset('Display','iter','TolX',1e-9))
...
%% file outputs
save('FWHM_Map_XQuartz_varykcdir=y_fixkz=6p5_fixka=6p5_f=1p11Mhz.mat')
dlmwrite('FWHM_Map_XQuartz_varykcdir=y_fixkz=6p5_fixka=6p5_f=1p11Mhz.txt',[lambda_perp_vect',x_FWHM',y_FWHM'],'delimiter','\t');
